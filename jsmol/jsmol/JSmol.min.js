...
"px",left:a._mousePageX+"px"})},c.mouseMove=function(c){this.isDragging&&a._mouseOwner==this&&(this.timestamp=System.currentTimeMillis(),this.container.css({top:this.pageY0+(c.pageY-this.pageY)+"px",left:this.pageX0+(c.pageX-this.pageX)+"px"}))},c.dragBind=function(c){this.applet._ignoreMouse=!c;this.container.unbind("mousemoveoutjsmol");this.container.unbind("touchmoveoutjsmol");this.container.unbind("mouseupoutjsmol");this.container.unbind("touchendoutjsmol");a._setMouseOwner(null);if(c){var b=
this;this.container.bind("mousemoveoutjsmol touchmoveoutjsmol",function(a,c,d){b.mouseMove(d)});this.container.bind("mouseupoutjsmol touchendoutjsmol",function(a,c,d){b.mouseUp(d)})}})};c.JSDialog=function(){};c.setDraggable(c.JSDialog);c.getScreenDimensions=function(a){a.width=m(window).width();a.height=m(window).height()};c.dispose=function(b){a.$remove(b.id+"_mover");delete c.htDialogs[b.id];b.container.obj.dragBind(!1)};c.register=function(a,d){a.id=d+ ++c.count;c.htDialogs[a.id]=a};c.setDialog=
function(b){a._setMouseOwner(null);a.$remove(b.id);var d=b.id+"_mover",e=a._$(d),g;e[0]?(e.html(b.html),g=e[0].jd):(a.$after("body","<div id='"+d+"' style='position:absolute;left:0px;top:0px;'>"+b.html+"</div>"),g=new c.JSDialog,e=a._$(d),b.container=e,g.applet=b.manager.vwr.applet,g.setContainer(e),g.dialog=b,g.setPosition(),g.dragBind(!0),e[0].jd=g);a.$bind("#"+b.id+" .JButton","mousedown touchstart",function(){g.ignoreMouse=!0});a.$bind("#"+b.id+" .JComboBox","mousedown touchstart",function(){g.ignoreMouse=
!0});a.$bind("#"+b.id+" .JCheckBox","mousedown touchstart",function(){g.ignoreMouse=!0});a.$bind("#"+b.id+" .JTextField","mousedown touchstart",function(){g.ignoreMouse=!0});a.$bind("#"+b.id+" .JTable","mousedown touchstart",function(){g.ignoreMouse=!0});a.$bind("#"+b.id+" .JScrollPane","mousedown touchstart",function(){g.ignoreMouse=!0});a.$bind("#"+b.id+" .JEditorPane","mousedown touchstart",function(){g.ignoreMouse=!0})};c.setSelected=function(c){a.$prop(c.id,"checked",!!c.selected)};c.setSelectedIndex=
function(c){a.$prop(c.id,"selectedIndex",c.selectedIndex)};c.setText=function(c){a.$prop(c.id,"value",c.text)};c.setVisible=function(c){a.$setVisible(c.id,c.visible)};c.setEnabled=function(c){a.$setEnabled(c.id,c.enabled)};c.click=function(b,d){var e=c.htDialogs[b.id];if(e){var g=e.toString();if(0<=g.indexOf("JCheck"))e.selected=b.checked;else if(0<=g.indexOf("JCombo"))e.selectedIndex=b.selectedIndex;else if(null!=e.text&&(e.text=b.value,d&&13!=(d.charCode||d.keyCode)))return}g=c.htDialogs[a.$getAncestorDiv(b.id,
"JDialog").id];g.manager.actionPerformed(e?e.name:g.registryKey+"/"+b.id)};c.setFront=function(c){var d=c.manager.vwr.applet;c.zIndex!=a._getZ(d,"dialog")&&(c.zIndex=a._incrZ(d,"dialog"));c.container&&((c.container[0]||c.container).style.zIndex=c.zIndex)};c.hideMenus=function(a){if(a=a._menus)for(var d in a)a[d].visible&&c.hideMenu(a[d])};c.windowClosing=function(b){b=c.htDialogs[a.$getAncestorDiv(b.id,"JDialog").id];b.registryKey?b.manager.processWindowClosing(b.registryKey):b.dispose()};a._track=
function(c){if(a._tracker){try{var d='<iframe style="display:none" width="0" height="0" frameborder="0" tabindex="-1" src="'+(a._tracker+"&applet="+c._jmolType+"&version="+a._version+"&appver="+self.___JmolVersion+"&url="+encodeURIComponent(document.location.href))+'"></iframe>';a.$after("body",d)}catch(e){}delete a._tracker}return c};a.getProfile=function(){window["j2s.doProfile"]=!0;if(self.Clazz&&self.JSON)return Clazz._profile||(Clazz._profile={}),Clazz.getProfile()};a._getInChIKey=function(a,
c){0<=c.indexOf("MOL=")&&c.split("MOL=")[1].split('"')};a._getAttr=function(a,c){var d=a.indexOf(c+"=");return 0<=d&&0<=(d=a.indexOf('"',d))?a.substring(d+1,a.indexOf('"',d+1)):null};a.User={viewUpdatedCallback:null};a.View={count:0,applets:{},sets:{}};(function(c){c.updateView=function(d,e){if(null!=d._viewSet){e.chemID||(d._searchQuery=null);e.data||(e.data="N/A");e.type=d._viewType;if(null==(d._currentView=c.__findView(d._viewSet,e)))d._currentView=c.__createViewSet(d._viewSet,e.chemID,e.viewID||
e.chemID);d._currentView[e.type].data=e.data;d._currentView[e.type].smiles=d._getSmiles();a.User.viewUpdatedCallback&&a.User.viewUpdatedCallback(d,"updateView");c.__setView(d._currentView,d,!1)}};c.updateFromSync=function(d,e){d._updateMsg=e;var g=a._getAttr(e,"sourceID")||a._getAttr(e,"file");if(g){var k=c.__findView(d._viewSet,{viewID:g});if(null==k)return a.updateView(d,e);k!=d._currentView&&c.__setView(k,d,!0);var r=(g=a._getAttr(e,"atoms"))&&0<=e.indexOf("selectionhalos ON")?eval("["+g+"]"):
[];setTimeout(function(){d._currentView==k&&c.updateAtomPick(d,r)},10);a.User.viewUpdatedCallback&&a.User.viewUpdatedCallback(d,"updateFromSync")}};c.updateAtomPick=function(c,b){var d=c._currentView;if(null!=d){for(var k in d)"info"!=k&&d[k].applet!=c&&d[k].applet._updateAtomPick(b);a.User.viewUpdatedCallback&&a.User.viewUpdatedCallback(c,"updateAtomPick")}};c.dumpViews=function(a){var d=c.sets[a];if(d){var e="View set "+a+":\n";a=c.applets[a];for(var k in a)e+="\napplet "+a[k]._id+" currentView="+
...
a.preventDefault();a.dataTransfer.dropEffect="copy"});a.$appEvent(c,"appletdiv","drop",function(d){d=d.originalEvent;d.stopPropagation();d.preventDefault();var e=d.dataTransfer.files[0];d=new FileReader;d.onloadend=function(d){if(d.target.readyState==FileReader.DONE){var k="cache://DROP_"+e.name;d=a._toBytes(d.target.result);c._applet.viewer.cacheFileByName("cache://DROP_*",!1);"JSV"==c._viewType||k.endsWith(".jdx")?a.Cache.put(k,d):c._applet.viewer.cachePut(k,d);c._applet.viewer.openFileAsyncSpecial(k,
...
arguments.length,b=(c-1)/2;0<=--b;){var f=arguments[--c],e=arguments[--c];a[e]=a.prototype[e]=f}};a.prepareFields=function(d,c){var b=[];if(null!=d.con$truct)for(var f=d.con$truct.stacks,e=0;e<f.length;e++)b[e]=f[e];a.addProto(d.prototype,"con$truct",d.con$truct=function(){var a=arguments.callee.stacks;if(null!=a)for(var c=0;c<a.length;c++)a[c].apply(this,[])});b[b.length]=c;d.con$truct.stacks=b;d.con$truct.index=0};a.registerSerializableFields=function(a){var c=arguments,b=c.length,e=[];if(null!=
...
function(a){q.consoleOutput("undefined"==typeof a?"\r\n":null==a?"null\r\n":a+"\r\n")};System.err.__CLASS_NAME__="java.io.PrintStream";System.err.print=function(a){q.consoleOutput(a,"red")};System.err.println=function(a){q.consoleOutput("undefined"==typeof a?"\r\n":null==a?"null\r\n":a+"\r\n","red")};a.setConsoleDiv=function(a){window["j2s.lib"]&&(window["j2s.lib"].console=a)};a.binaryFolders=a._Loader.binaryFolders=[a._Loader.getJ2SLibBase()]}(Clazz))};___JmolDate="$Date: 2014-06-09 09:04:30 -0500 (Mon, 09 Jun 2014) $";
___fullJmolProperties="src/org/jmol/viewer/Jmol.properties";___JmolVersion="14.0.17";
